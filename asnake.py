...
#print(e.keysym)#Tells us which key has been pressed on the keyboard
    ...
    #MOVING THE SNAKE
def move():
    global snake,GAME_OVER,snake_body,food,SCORE,HIGH_SCORE,play
    if(GAME_OVER):
        time.sleep(2)#DELAY SCREEN
        start()
        return
    ...
    if(snake.x==food.x and snake.y==food.y):#x and y position of the snake is equal  to x and y position of the food
...
def draw():
    ...
    canvas.create_rectangle(snake.x, snake.y, snake.x+TILE_SIZE, snake.y+TILE_SIZE, fill="purple")#SNAKE HEAD
    for tile in snake_body:
        canvas.create_rectangle(tile.x,tile.y,tile.x+TILE_SIZE,tile.y+TILE_SIZE,fill="blue")#MAKING THE SNAKE BODY

    if (GAME_OVER):
        canvas.delete("all")
        if(SCORE<100):
            canvas.create_text(WINDOW_WIDTH/2,WINDOW_HEIGHT/2,font="Arial 20 bold",text=f"JUST WATCH MOVIES INSTEAD😒 \nYOUR SCORE IS {SCORE} HIGH SCORE:{HIGH_SCORE}",fill="red")
        else:
        ...
        canvas.create_text(300,20,font="Arial 20 bold",text=f"SCORE:{SCORE} HIGH SCORE:{HIGH_SCORE}",fill="white")
    ...
    if SCORE >= 100:
        ...
        if SCORE == 100:
            canvas.create_text(300, 80, font="Arial 20 bold", text="CONGRATULATIONS😊 100!", fill="red",tags="text")
